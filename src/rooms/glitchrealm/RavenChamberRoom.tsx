  ...
  useEffect(() => {
    const handleCommand = (event: CustomEvent) => {
      ...
      if (
        ...
        command.includes("look console") ||
        command.includes("examine console")
      ) {
        dispatch({
          type: "RECORD_MESSAGE",
          payload: {
    ...
    return () =>
      window.removeEventListener("gameCommand", handleCommand as EventListener);
  }, [dispatch]);
    ...
    <div className="raven-chamber-room">
      <div className="room-description">
        <p className="narrative-text">
          You step into a narrow chamber humming with quiet static. Cracked
          screens line the walls, some looping nonsense glyphs, others frozen
          mid-boot. At the far end, a sunken pedestal glows faintly blue.
        </p>

        {!hasInteracted && (
          <p className="narrative-text">
            As you approach, an ancient AI console flickers to life, casting
            harsh white light. A synthetic voice—fragmented and uncertain—calls
            out to you, mistaking you for someone else.
          </p>
        )}

        {isPlayerRedacted && (
          <p className="glitch-text error-text">
            ⚠️ The chamber seems to pulse with recognition. Warning lights blink
            silently. You feel watched by unseen systems.
          </p>
        )}
      </div>
        ...
        {!isConsoleActive && (
          <div className="console-prompt">
            <p className="action-hint">
              💡 You can <strong>interact console</strong> or{" "}
              <strong>access archive</strong> to activate the terminal.
            </p>
          </div>
        )}
