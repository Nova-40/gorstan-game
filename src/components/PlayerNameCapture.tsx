...
const GorstanIcon = () => (
  <img
    src="/public/images/gorstanicon.png"
...
const PlayerNameCapture: React.FC<{ onNameSubmit: (name: string) => void }> = ({ onNameSubmit }) => {
  ...
  // Keyboard navigation for modal screens (arrows and ESC)
  useEffect(() => {
    if (!modal) return;
  ...
  // Ctrl+Click or Cmd+Click opens cheat modal, normal click opens instructions
  const handleInstructionsClick = (e: React.MouseEvent) => {
    if (e.ctrlKey || e.metaKey) {
      setModal("cheat");
      // Enable debug mode when Ctrl+click is used
