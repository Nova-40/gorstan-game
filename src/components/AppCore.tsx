...
const AppCore: React.FC = () => {
  ...
  const [transitionTargetRoom, setTransitionTargetRoom] =
    useState<string>("controlnexus");
  ...
  const [roomTransitionActive, setRoomTransitionActive] =
  ...
  const lookModalTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const [currentGuidance, setCurrentGuidance] =
    useState<AIGuidanceResponse | null>(null);
  ...
  const hintCheckIntervalRef = useRef<ReturnType<typeof setInterval> | null>(
    null,
  );
  ...
  const [lastShownRoomDescription, setLastShownRoomDescription] = useState<
    string | null
  >(null);
  const [roomFallbackAttempted, setRoomFallbackAttempted] =
    useState<boolean>(false);
      ...
      .map((npcOrId: NPC | string) => {
        if (typeof npcOrId === "string") {
          ...
          return {
            id: npcOrId,
            name:
              npcOrId.charAt(0).toUpperCase() +
              npcOrId.slice(1).replace(/_/g, " "),
  ...
  const roomMap: Record<string, Room> = useMemo(
    () => state.roomMap || {},
    [state.roomMap],
  );
  ...
  const { handleLibrarian } = useLibrarianLogic(
    state,
    dispatch,
    room,
    loadModule,
  ...
  const openModal = useCallback(
    (name: OpenModalType): void => setModal(name),
    [],
  );
    ...
    (slotId: string) => {
      try {
        ...
        if (savedData) {
          ...
          dispatch({
            type: "ADVANCE_STAGE",
            payload: saveData.gameState.stage,
          });
          dispatch({
            type: "SET_PLAYER_NAME",
            payload: saveData.gameState.player.name,
          });
          dispatch({
            type: "MOVE_TO_ROOM",
            payload: saveData.gameState.currentRoomId,
          });
    ...
    const generateNPCBehaviors = async () => {
      ...
      for (const npc of npcsInRoom) {
        try {
          ...
          console.warn(
            `[NPC AI] Behavior generation failed for ${npc.id}:`,
            error,
          );
        }
      }
  ...
  useEffect(() => {
    if (state.flags?.openNPCConsole) {
      ...
      if (targetNPC) {
        ...
        dispatch({
          type: "SET_FLAG",
          payload: { flag: "openNPCConsole", value: null },
        });
      }
    }
  ...
  useEffect(() => {
    if (state.flags?.triggerTeleport) {
      ...
      dispatch({
        type: "SET_FLAG",
        payload: { flag: "triggerTeleport", value: null },
      });
    }
  ...
  const handleLookAround = useCallback((): void => {
    ...
    const roomTitle: string =
      (room as any).title || (room as any).name || "Unknown room";
  ...
  const checkForHints = useCallback(
    async (cmd: string, lowerCmd: string) => {
      ...
      const hasRepeatedCommands =
        recentCommands.filter((c) => c === cmd).length >= 2;
      const hasVariousFailedAttempts = recentCommands.length >= 3;

      if (
        hasFailureMessage ||
        hasRepeatedCommands ||
        hasVariousFailedAttempts
      ) {
        try {
          ...
          const unifiedGuidance =
            await unifiedAI.getUnifiedGuidance(unifiedContext);
          ...
          const hintResponse =
            await aylaHintSystem.shouldAylaInterrupt(context);
  ...
  const handleCommand = useCallback(
    (cmd: string): void => {
      ...
      const commandSuccess: boolean = false;
      ...
      if (lowerCmd.startsWith("talk to ")) {
        ...
        const match: NPC | string | undefined = npcsInRoom.find(
          (npc: NPC | string) => {
            const name: string =
              typeof npc === "object" && "name" in npc
                ? npc.name
                : (npc as string);
        ...
        if (match) {
          const npcId: string =
            typeof match === "object" && "id" in match
              ? match.id
              : (match as string);
      ...
      selectedItems.forEach((item: string) => {
        if (item === "Run Bag") {
          ...
          import("../engine/dominicPickupConversation").then((mod) => {
            const preventPickup = mod.handleDominicPickupAttempt(
              state,
              dispatch,
            );
  ...
  const transitionInfo = useRoomTransition(
    previousRoom,
    room,
    lastMovementAction,
  );
  ...
  const directionRoomTitles = useMemo(
    () => ({
      north: room?.exits?.north
        ? (roomMap[room.exits.north]?.title ?? room.exits.north)
        : "",
      south: room?.exits?.south
        ? (roomMap[room.exits.south]?.title ?? room.exits.south)
        : "",
      east: room?.exits?.east
        ? (roomMap[room.exits.east]?.title ?? room.exits.east)
        : "",
      west: room?.exits?.west
        ? (roomMap[room.exits.west]?.title ?? room.exits.west)
        : "",
      jump: room?.exits?.jump
        ? (roomMap[room.exits.jump]?.title ?? room.exits.jump)
        : "",
      sit: room?.exits?.sit
        ? (roomMap[room.exits.sit]?.title ?? room.exits.sit)
        : "",
      up: room?.exits?.up
        ? (roomMap[room.exits.up]?.title ?? room.exits.up)
        : "",
      down: room?.exits?.down
        ? (roomMap[room.exits.down]?.title ?? room.exits.down)
        : "",
      back: room?.exits?.back
        ? (roomMap[room.exits.back]?.title ?? room.exits.back)
        : "",
      out: room?.exits?.out
        ? (roomMap[room.exits.out]?.title ?? room.exits.out)
        : "",
    }),
    [room?.exits, roomMap],
  );
  ...
  useEffect(() => {
    if (!room && currentRoomId && !roomFallbackAttempted) {
      ...
      dispatch({
        ...
        payload: { currentRoomId: "controlnexus" },
      });
  ...
  useEffect(() => {
    if (stage.startsWith("transition_")) {
      ...
      console.log(
        "[AppCore] Setting transition type from stage:",
        stage,
        "→",
        type,
      );
    ...
    if (!state.roomMap || Object.keys(state.roomMap).length === 0) {
      try {
        ...
        if (!loadedRoomMap || Object.keys(loadedRoomMap).length === 0) {
          console.warn(
            "[AppCore] No rooms loaded from main loader, using fallback rooms",
          );
        ...
        console.log(
          "[AppCore] Loading room map with",
          Object.keys(loadedRoomMap).length,
          "rooms",
        );
        ...
        try {
          ...
          console.warn(
            "[AppCore] Using fallback rooms due to initialization error",
          );
        ...
        if (npcsInRoom.length > 1) {
          import("../npc/groupChatLogic").then(({ GroupChatManager }) => {
            if (GroupChatManager.shouldForceGroupChat(room.id, currentZone)) {
              dispatch({
                type: "SET_FLAG",
                payload: { flag: "forceGroupChat", value: true },
              });

              // Auto-open group conversation for Stanton Harcourt zone
              if (
                currentZone === "stantonZone" ||
                currentZone === "stantonharcourtZone"
              ) {
                setTimeout(() => {
                  setIsGroupConversation(true);
                  setSelectedNPC(npcsInRoom[0] || null);
  ...
  if (
    !state.currentRoomId ||
    !state.roomMap ||
    !state.roomMap[state.currentRoomId]
  ) {
    ...
    return (
      <TeletypeIntro
        ...
        onComplete={(data: IntroCompletionData) => {
          ...
          if (data.targetRoom) {
            console.log(
              "[AppCore] Setting transition target room:",
              data.targetRoom,
            );
          ...
          if (data.inventoryBonus) {
            console.log(
              "[AppCore] Setting transition inventory:",
              data.inventoryBonus,
            );
        ...
        <QuickActionsPanel
          ...
          onJump={() => {
            ...
            if (jumpRoomId) {
              ...
              console.warn(
                "🚫 Jump exit not found from current room:",
                state.currentRoomId,
              );
            }
          }}
          onMove={(direction: string) => {
            ...
            const currentRoom = state.roomMap[state.currentRoomId];
            console.log(
              "[AppCore] Current room from state.roomMap:",
              currentRoom,
            );
          ...
          onSit={() => {
            ...
            if (sitRoomId) {
              ...
              console.warn(
                "🚫 Sit exit not found from current room:",
                state.currentRoomId,
              );
            }
          }}
