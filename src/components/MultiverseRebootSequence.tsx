...
const rebootSequence: RebootMessage[] = [
  ...
  {
    text: "Constructing multiversal constants...",
    delay: 3000,
    type: "system",
  },
  { text: "Recompiling narrative entropy...", delay: 3500, type: "system" },
  {
    text: "Rebalancing protagonist probability...",
    delay: 4000,
    type: "system",
  },
...
const MultiverseRebootSequence: React.FC = () => {
      ...
      timeoutId = setTimeout(() => {
        ...
        if (currentMessageIndex === rebootSequence.length - 1) {
          setTimeout(() => {
            ...
            import("../logic/achievementEngine").then(
              ({ unlockAchievement }) => {
  ...
  useEffect(() => {
    if (
      state.flags?.multiverse_reboot_pending &&
      !state.flags?.multiverse_reboot_active
    ) {
      setFadeStage("fading");
