          ...
          {activeTab === "ai" && (
            ...
            >
              <div className="text-slate-300 text-xs">
                    ...
                    {aiStats
                      ? Math.round(
                          (aiStats.successfulResponses /
                            Math.max(aiStats.totalRequests, 1)) *
                            100,
                        )
                      : 0}
                    %
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-yellow-400">
                    {aiStats
                      ? Math.round(aiStats.playerSatisfactionScore * 100)
                      : 0}
                    %
                  </span>
                </div>
              ...
              <div className="border-t border-slate-700 pt-2">
                <h4 className="text-slate-200 text-xs font-medium mb-1">
                  Recent Activity
                </h4>
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {updates
                    ...
                    .map((update, index) => (
                      <div
                        key={index}
                        className="text-xs text-slate-400 p-1 bg-slate-800 rounded"
                      >
            ...
            >
              <div className="text-slate-300 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="text-blue-400">
                    {npcStats?.totalNPCs || 0}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>AI Enabled:</span>
                  <span className="text-green-400">
                    {npcStats?.aiEnabledNPCs || 0}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Wandering:</span>
                  <span className="text-yellow-400">
                    {npcStats?.wanderingNPCs || 0}
                  </span>
                </div>
              </div>
              ...
              <div className="border-t border-slate-700 pt-2">
                ...
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {Object.entries(npcBehaviors).map(
                    ([npcId, behavior], index) => (
                      <div
                        key={index}
                        className="text-xs p-1 bg-slate-800 rounded"
                      >
          ...
          {activeTab === "report" && (
            ...
            >
              ...
              <div className="text-xs text-slate-400">
                <p>
                  Generate a comprehensive AI usage and gameplay report in the
                  console.
                </p>
