...
const EnhancedNPCConsole: React.FC<EnhancedNPCConsoleProps> = ({
  ...
  const realNpcs = npcs.filter(
    (npc) => npc.id !== "group_conversation" && npc.id !== "group_chat",
  );

  // Get primary NPC from real NPCs only
  const primaryNpc =
    realNpcs.find((npc) => npc.id === activeNpcId) || realNpcs[0];
  ...
  const getNpcAvatar = (npcId: string): string => {
    ...
    const avatarUrl =
      npc?.portrait || npcImages[npcId] || "/images/fallback.png";

    // Debug logging to help identify issues
    if (!npc?.portrait && !npcImages[npcId]) {
      console.log(
        `[NPC Avatar] No image found for NPC: ${npcId}, using fallback`,
      );
    }
  ...
  const getSpeakerDisplay = (
    message: EnhancedDialogueMessage,
  ): React.ReactElement => {
    ...
    const competitiveMarker = getCompetitiveMarker(
      message.npcId!,
      message.speakerContext,
    );
  ...
  useEffect(() => {
    if (isOpen && realNpcs.length > 0) {
      if (isGroupConversation && realNpcs.length > 1) {
        ...
        if (
          GroupChatManager.shouldForceGroupChat(
            state.currentRoomId,
            currentZone,
          )
        ) {
  ...
  const handleSendMessage = () => {
    ...
    if (
      ...
      dispatch({
        type: "SET_ALLIANCE",
        payload: { ally: "morthos", points: 20 },
      });
    } else if (
    ...
    const isRudeToWendell = (
      message: string,
      npcs: typeof realNpcs,
    ): boolean => {
      const hasWendell = npcs.some(
        (npc) =>
          npc.id === "mr_wendell" || npc.name.toLowerCase().includes("wendell"),
      );
    ...
    if (isRudeToWendell(trimmed, realNpcs)) {
      ...
      setTimeout(() => {
        dispatch({
          type: "SET_FLAG",
          payload: { flag: "multiverse_reboot_pending", value: true },
        });
      }, 3000);
    ...
    if (isGroupConversation) {
      ...
      console.log(
        `[Enhanced NPC Console] Group chat - ${respondingNpcId} will respond`,
      );
    } else {
    ...
    setTimeout(
      async () => {
        try {
          ...
          console.error(
            "[Enhanced NPC Console] Error in response handling:",
            error,
          );
  ...
  const determineRespondingNpc = (message: string): string => {
    ...
    if (
      ...
      const totalWeight = Object.values(npcWeights).reduce(
      ...
      for (const npcId of availableNpcs) {
        ...
        if (weight) {
          ...
          if (random <= 0) {
            console.log(
              `[Enhanced NPC Console] 🎲 Weighted random selection chose: ${npcId}`,
            );
            return npcId;
          }
        }
    ...
    const randomNpc =
      fallbackNpcs[Math.floor(Math.random() * fallbackNpcs.length)];
    console.log(
      `[Enhanced NPC Console] 🎯 Fallback random selection chose: ${randomNpc}`,
    );
    return randomNpc;
  };
  ...
  const handleGroupResponse = async (playerMessage: string) => {
    ...
    if (respondingNpc) {
      try {
        ...
        setMessages((prev) => [...prev, npcMessage]);
        console.log(
          `[Enhanced NPC Console] 💬 Added message to state from ${respondingNpc.name}`,
        );
            ...
            async () => {
              ...
              if (reactionNpcId) {
                ...
                const reactionMessage: EnhancedDialogueMessage = {
                  ...
                  npcId: reactionNpcId,
                  npcName:
                    realNpcs.find((n) => n.id === reactionNpcId)?.name ||
                    "Unknown",
  ...
  const handleSingleResponse = async (playerMessage: string) => {
    ...
    try {
      ...
      console.log(
        `[Enhanced NPC Console] Got response for ${primaryNpc?.name}: ${response}`,
      );
  ...
  const generateContextualResponse = async (
    ...
    try {
      console.log(
        `[Enhanced NPC Console] 🤖 Attempting AI response for ${npcId}`,
      );

      // Create a shorter timeout for faster fallback
      const enhancedResponsePromise = getEnhancedNPCResponse(
        npcId,
        playerMessage,
        state,
      );
    ...
    console.log(
      `[Enhanced NPC Console] � Using scripted response for ${npcId}`,
    );
    ...
    const getScriptedResponse = (
      ...
      const selectUniqueResponse = (responses: string[]): string => {
        const availableResponses = responses.filter(
          (response) =>
            !npcHistory.includes(response) ||
            npcHistory.length >= responses.length,
        );
      ...
      switch (npcId) {
        ...
        case "morthos":
          ...
          if (
            lowerMessage.includes("al") ||
            lowerMessage.includes("bureaucrat")
          ) {
        ...
        case "librarian":
          ...
          if (
            lowerMessage.includes("lattice") ||
            lowerMessage.includes("multiverse")
          ) {
    ...
    console.log(
      `[Enhanced NPC Console] 📝 Scripted response for ${npcId}: ${scriptedResponse}`,
    );
  ...
  const generateNPCToNPCResponse = async (
    ...
    try {
      ...
      if (aiResponse) {
        console.log(
          `[NPC-to-NPC] ✅ ${speakingNpcId} → ${targetNpcId}: ${aiResponse}`,
        );
        return aiResponse;
      }
    } catch (error) {
      console.warn(
        `[NPC-to-NPC] ❌ AI failed for ${speakingNpcId} → ${targetNpcId}:`,
        error,
      );
    }
  ...
  const generateInterNPCResponse = async (
    ...
    try {
      ...
      console.warn(
        `Enhanced inter-NPC response failed for ${reactingNpcId}:`,
        error,
      );
    }
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div
        className="modal enhanced-npc-console"
          ...
          <div className="messages-container">
            {messages.map((message) => (
              <div
                ...
                data-npc-id={
                  message.speaker === "npc" ? message.npcId : undefined
                }
                style={
                  message.speaker === "npc"
                    ? {
                ...
                <div className="message-header">
                  <div className="speaker-info npc">
                    ...
                    <span className="speaker-name">
                      {typingNpcId
                        ? realNpcs.find((n) => n.id === typingNpcId)?.name
                        : "Someone"}{" "}
                      is typing...
                    </span>
                  </div>
          ...
          <div className="input-area">
            ...
            {isGroupConversation && (
              <div className="conversation-hints">
                <small>
                  💡 Tip: Address NPCs by name (e.g., "Al, what do you think?")
                  or ask topic-based questions
                </small>
              </div>
            )}
          </div>
