...
const TrapManagementModal: React.FC<TrapManagementModalProps> = ({
  ...
  const handleAnalyzeTrap = useCallback(() => {
    ...
    analyzeTimeoutRef.current = setTimeout(() => {
      ...
      setManagementState((prev) => ({
        ...
        isAnalyzing: false,
        hasAnalyzed: true,
        disarmMethod: disarmResult.canDisarm
          ? disarmResult.method || null
          : null,
        playerActions: [
          ...prev.playerActions,
  ...
  const handleDisarmAttempt = useCallback(() => {
    ...
    disarmTimeoutRef.current = setTimeout(() => {
      ...
      if (success) {
        const disarmed = disarmTrap(
          currentRoomId,
          disarmResult.method || "unknown",
        );
        if (disarmed) {
          dispatch({
            type: "ADD_MESSAGE",
  ...
  const handleDisarmFailure = useCallback(() => {
    ...
    setManagementState((prev) => ({
      ...
      playerActions: [
        ...prev.playerActions,
        "💀 Disarmament failed! The trap triggered!",
      ],
    }));
  ...
  return (
    <Modal
      visible={isOpen}
      onClose={onClose}
      title="⚠️ Trap Management"
      pinned={true}
    >
      <div className="trap-management-modal">
        {/* Timer Display */}
        <div
          className={`timer-display ${managementState.timeRemaining <= 10 ? "urgent" : ""}`}
        >
          <Clock size={24} />
        ...
        <div className="action-buttons">
          <button
            ...
            disabled={
              managementState.isAnalyzing || managementState.hasAnalyzed
            }
          >
            <Search size={16} />
            {managementState.isAnalyzing
        ...
        <div className="help-text">
          <p>
            ⚠️ You have {managementState.timeRemaining} seconds before the trap
