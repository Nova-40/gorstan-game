...
const RavenInteraction: React.FC<RavenInteractionProps> = ({
  onComplete,
  playerName,
}) => {
  const { dispatch } = useGameState();
  const [phase, setPhase] = useState<
    "greeting" | "prompt" | "displaying" | "glitch" | "complete"
  >("greeting");
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("gameCommand", handleInput as EventListener);
      ...
      {phase === "prompt" && (
        <div className="raven-prompt">
          <p className="action-hint">
            💡 Type <strong>Y</strong> or <strong>N</strong> to respond to
            R.A.V.E.N.
          </p>
        </div>
      )}
      ...
      {phase === "displaying" && (
        <div className="raven-display">
          <p className="glitch-text">
            📡 Streaming classified data... {displayedEntries}/
            {REDACTED_REGISTER.length}
          </p>
        </div>
      )}
