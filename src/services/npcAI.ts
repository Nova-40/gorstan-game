...
export class NPCAIController {
  ...
  private npcStates: Map<
    string,
    { location: string; lastAction: number; mood: string }
  > = new Map();
   ...
   */
  public async generateNPCBehavior(
    ...
    try {
      ...
      console.warn(
        `[NPC AI] Behavior generation failed for ${npcProfile.npcId}:`,
        error,
      );
      return null;
    }
   ...
   */
  private async generatePikeCoffeeCallout(
    context: NPCBehaviorContext,
  ): Promise<NPCAction | null> {
   ...
   */
  private async generateContextualBehavior(
    context: NPCBehaviorContext,
  ): Promise<NPCAction | null> {
    const { npcProfile, currentRoom, playerPresent, recentPlayerActions } =
      context;
...
${npcProfile.name}'s response:`;
  ...
  private getLoreContext(npcId: string): string {
    ...
    return (
      loreMap[npcId] ||
      "A character with their own role in the multiverse's grand design"
    );
  }
  ...
  private updateNPCMemory(npcId: string, context: NPCBehaviorContext): void {
    ...
    if (relevantActions.length > 0) {
      ...
      if (profile.memory.playerPreferences.length > 20) {
        profile.memory.playerPreferences =
          profile.memory.playerPreferences.slice(-10);
      }
    }
  }
  ...
  public getNPCState(
    npcId: string,
  ): { location: string; lastAction: number; mood: string } | null {
  ...
  private buildCharacterStoryPrompt(
    ...
    if (npcProfile.npcId === "mrwendell") {
      ...
      if (aboutCharacter.toLowerCase().includes("polly")) {
        ...
        if (
          playerAction?.includes("killed") &&
          playerAction?.includes("dominic")
        ) {
  ...
  public getStats(): {
    ...
    return {
      ...
      aiEnabledNPCs: profiles.filter((p) =>
        Object.values(p.aiFeatures).some((f) => f),
      ).length,
      wanderingNPCs: profiles.filter((p) => p.aiFeatures.wanderingAI).length,
      totalInteractions: profiles.reduce(
        (sum, p) => sum + p.memory.playerInteractions,
        0,
      ),
    };
  }
