 ...
 */
export const CelebrationController: React.FC<CelebrationControllerProps> = ({
  children,
}) => {
  ...
  const checkActiveCelebrations = async () => {
    try {
      ...
      const currentCelebration =
        filteredCelebrations.length > 0 ? filteredCelebrations[0] : null;
  ...
  const extractTraditionFromCelebration = (celebration: Span): string => {
    ...
    if (
      label.includes("rosh") ||
      label.includes("passover") ||
      label.includes("hanukkah")
    )
      {return "jewish";}
    ...
    if (
      label.includes("solstice") ||
      label.includes("equinox") ||
      label.includes("naw-rúz")
    )
      {return "seasonal";}
    ...
    <>
      ...
      {state.currentCelebration && (
        <CelebrationOverlay
          ...
          onDismiss={() =>
            handleDismissCelebration(state.currentCelebration!.id)
          }
        />
      )}
    </>
...
function extractTraditionFromLabel(label: string): string {
  ...
  if (
    lower.includes("solstice") ||
    lower.includes("equinox") ||
    lower.includes("naw-rúz")
  )
    {return "seasonal";}
