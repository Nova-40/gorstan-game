...
export function morthosToAl(text: string, ctx: Ctx): void {
  if (
    !canStartConversation(NPC_IDS.MORTHOS, NPC_IDS.AL, ctx.roomId, ctx.state)
  ) {
    return;
  }
...
export function alToMorthos(text: string, ctx: Ctx): void {
  if (
    !canStartConversation(NPC_IDS.AL, NPC_IDS.MORTHOS, ctx.roomId, ctx.state)
  ) {
    return;
  }
...
export const NPCTalk = {
  ...
  aylaToMorthos: (text: string, ctx: Ctx) =>
    aylaSayTo(NPC_IDS.MORTHOS, text, ctx),
  ...
  // Generic
  any: (from: string, to: string, text: string, ctx: Ctx) =>
    npcSayTo(from, to, text, ctx),
};
