...
export function checkProactivePrompts(
  ...
  for (const npcId of availableNPCs) {
    ...
    if (
      currentTime - lastPrompt < 15000 ||
      proactiveState.suppressionFlags.has(suppressionKey)
    ) {
      continue;
    }

    const prompt = evaluateNPCPrompt(
      npcId,
      context,
      npcMemoryStates[npcId] || createEmptyMemory(),
    );
...
function evaluateAylaPrompts(
  ...
  if (
    context.sessionInfo.roomVisitCount > 5 &&
  ...
  if (
    ...
    return {
      ...
      accessibility: {
        screen_reader_text:
          "Ayla is urgently trying to get your attention - time is running out!",
  ...
  if (
    context.recentConsoleEvents.includes("lore_discovered") &&
    memory.relationshipLevel > 0.5
  ) {
    return {
      npcId: "ayla",
      type: "visual_flash",
      ...
      accessibility: {
        screen_reader_text:
          "Ayla seems to have connected what you just learned to something else",
      },
    };
  }
  ...
  if (
    context.inventory.includes("coin") &&
    context.roomId.includes("library")
  ) {
    return {
      npcId: "ayla",
      type: "visual_flash",
      ...
      accessibility: {
        screen_reader_text:
          "Ayla has noticed you have the coin and seems to want to discuss it",
      },
    };
  }
  ...
  if (timeRemaining < 30000) {
    return {
      ...
      accessibility: {
        ...
        keyboard_hint:
          "Press Tab to focus on Polly immediately - critical situation",
      },
    };
  }
...
function evaluateWendellLibraryPrompts(
  ...
  if (
    context.recentConsoleEvents.includes("book_examined") &&
    memory.totalInteractions < 3
  ) {
    return {
      npcId: "wendell",
      type: "visual_flash",
...
function evaluateChefPrompts(
  ...
  if (context.recentConsoleEvents.includes("cooking_failed")) {
    return {
      ...
      accessibility: {
        screen_reader_text:
          "Chef has noticed your cooking attempt failed and might have advice",
      },
    };
  }
