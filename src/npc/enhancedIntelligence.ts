...
class NPCIntelligenceEngine {
  ...
  generateResponse(
    ...
    const response = this.craftResponse(
      npc,
      memory,
      profile,
      playerInput,
  ...
  private updateRelationship(
    ...
    memory.relationship = Math.max(
      -1,
      Math.min(1, memory.relationship + adjustment),
    );
  ...
  private selectBaseResponse(
    ...
    const intentResponses =
      responses[intent as keyof typeof responses] || responses.conversation;

    // Avoid repeating recent responses
    const availableResponses = intentResponses.filter(
      (r) => r !== memory.lastResponse,
    );
    const selectedResponses =
      availableResponses.length > 0 ? availableResponses : intentResponses;

    return selectedResponses[
      Math.floor(Math.random() * selectedResponses.length)
    ];
  }
  ...
  shouldInitiateConversation(npcId: string, state: LocalGameState): boolean {
    ...
    if (memory.interactions > 0) {
      const timeSinceLastInteraction =
        Date.now() - (memory as any).lastInteractionTime;
  ...
  generateProactiveConversation(
    ...
    const proactiveMessages = {
      ...
      survivor:
        "There's something you should know about your current situation.",
    };

    const archetype =
      Object.keys(NPC_INTELLIGENCE_PROFILES).find(
        (key) =>
          NPC_INTELLIGENCE_PROFILES[
            key as keyof typeof NPC_INTELLIGENCE_PROFILES
          ] === profile,
      ) || "guide";
...
export function shouldNPCInitiateConversation(
  npcId: string,
  state: LocalGameState,
): boolean {
  const engine = NPCIntelligenceEngine.getInstance();
