...
export class EnhancedAylaConversation {
  ...
  analyzeGameState(context: ConversationContext): AylaAnalysis {
    ...
    const playerStuckLevel = this.calculateStuckLevel(
      playerActions,
      currentRoom,
      gameState,
    );
    ...
    const npcConflictLevel = this.analyzeNPCConflicts(
      npcsPresent,
      recentMessages,
    );

    // Analyze gameplay frustration
    const gameplayFrustration = this.analyzePlayerFrustration(
      recentMessages,
      playerActions,
    );
  ...
  private calculateStuckLevel(
    ...
    const roomRevisits = recentRooms.filter(
      (room: string) => room === currentRoom,
    ).length;
    if (roomRevisits > 2) {
      stuckScore += 0.4;
    }
  ...
  private analyzeNPCConflicts(
    ...
    if (npcsPresent.includes("al") && npcsPresent.includes("morthos")) {
      ...
      const competitiveCount = recentMessages.reduce((count, msg) => {
        return (
          count +
          competitiveWords.filter((word) => msg.toLowerCase().includes(word))
            .length
        );
      }, 0);
  ...
  private analyzePlayerFrustration(
    recentMessages: string[],
    playerActions: string[],
  ): number {
  ...
  private generateMediationResponse(
    npcsPresent: string[],
    context: ConversationContext,
  ): string {
    if (npcsPresent.includes("al") && npcsPresent.includes("morthos")) {
      ...
      return mediationResponses[
        Math.floor(Math.random() * mediationResponses.length)
      ];
    }
