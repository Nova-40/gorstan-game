...
export class ControlRoomEncounterOrchestrator {
   ...
   */
  private recallRelevantMemories(
    ...
    const uniqueMemories = allMemories.filter(
      (memory, index, self) =>
        index ===
        self.findIndex((m) => m.sourceEvent.id === memory.sourceEvent.id),
    );
  ...
  private determineEncounterType(
    ...
    if (recentInteraction === "betrayal" || recentInteraction === "conflict") {
      ...
      const hasReconciliation = memories.some(
        (m) => m.sourceEvent.type === "reconciliation",
      );
  ...
  private generateReunionDialogue(
    ...
    if (mostRelevantMemory) {
      const memoryDialogue =
        this.generateMemoryBasedDialogue(mostRelevantMemory);
  ...
  private generateConfrontationDialogue(
    ...
    const betrayalMemory = memories.find(
      (m) => m.sourceEvent.type === "betrayal",
    );
   ...
   */
  private generateMemoryBasedDialogue(
        ...
        case "flag-set":
          ...
          if (outcome.gameStateChanges) {
            ...
            if (gameState && gameState.flags) {
              ...
              console.warn(
                "Could not apply game state changes - game state not available",
              );
            }
          }
          break;
