...
export function maybeStartBanter(
  ...
  const morthosPresent = state.npcsInRoom?.some(
    (n) => n.id === NPC_IDS.MORTHOS,
  );
...
export function maybeAylaIntervention(
  state: LocalGameState,
  dispatch: Dispatch<GameAction>,
  roomId: string,
): void {
  ...
  if (morthosPresent && roomId === "controlnexus") {
    ...
    aylaSayTo(
      NPC_IDS.AL,
      "Systematic analysis might clarify their confusion.",
      ctx,
    );
  }
}
    ...
    case "puzzle_attempt_failed":
      ...
      if (Math.random() < 0.4) {
        ...
        NPCTalk.morthosAndAl.alStarts(
          "Systematic approach would be more efficient.",
          ctx,
        );
      }
      break;
...
export function scriptedConversations(
  state: LocalGameState,
  dispatch: Dispatch<GameAction>,
  roomId: string,
): void {
  ...
  if (
    state.flags.hasRemote &&
    !state.flags.triedChair &&
    roomId === "controlnexus"
  ) {
    // Hidden coordination
    NPCTalk.hiddenHint(
