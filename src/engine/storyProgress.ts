...
export function setFlag(
  ...
  try {
    ...
    console.log(
      `[StoryProgress] Flag set: ${flag} (category: ${normalizedCategory})`,
    );
    return true;
  } catch (error) {
...
export function setFlagDependencies(
  flag: string,
  dependencies: string[],
): boolean {
  try {
    if (!validateFlagName(flag) || !Array.isArray(dependencies)) {
      console.warn(
        "[StoryProgress] Invalid dependencies provided for flag:",
        flag,
      );
      return false;
    }
    ...
    const validDependencies = dependencies.filter((dep) =>
      validateFlagName(dep),
    );
    if (validDependencies.length !== dependencies.length) {
      console.warn(
        "[StoryProgress] Some dependencies are invalid for flag:",
        flag,
      );
    }
    ...
    if (hasCircularDependency(flag, validDependencies)) {
      console.warn(
        "[StoryProgress] Circular dependency detected for flag:",
        flag,
      );
      return false;
    }
    ...
    console.log(
      `[StoryProgress] Dependencies set for ${flag}: [${validDependencies.join(", ")}]`,
    );
    return true;
  } catch (error) {
...
export function registerEventTrigger(
  ...
  try {
    ...
    if (callbacks.length >= MAX_EVENT_TRIGGERS) {
      console.warn(
        `[StoryProgress] Maximum event triggers reached for flag: ${flag}`,
      );
      return false;
    }
  ...
  try {
    ...
    callbacks.forEach((callback, index) => {
      try {
        ...
        console.error(
          `[StoryProgress] Error in event callback ${index} for flag ${flag}:`,
          error,
        );
      }
    });
...
export function cleanupExpiredFlags(): number {
  try {
    ...
    if (expiredFlags.length > 0) {
      console.log(
        `[StoryProgress] Cleaned up ${expiredFlags.length} expired flags`,
      );
    }
...
export function loadFlags(): boolean {
  try {
    ...
    if (state.version !== MODULE_VERSION) {
      console.log(
        `[StoryProgress] Migrating from version ${state.version} to ${MODULE_VERSION}`,
      );
      migrateState(state);
    }
  ...
  try {
    ...
    const state: StoryProgressState = {
      ...
      categories: Object.fromEntries(
        Object.entries(categorizedFlags).map(([key, set]) => [
          key,
          Array.from(set),
        ]),
      ),
    ...
    try {
      ...
      console.warn(
        "[StoryProgress] localStorage quota exceeded, cleaning up...",
      );
      cleanupOldData();
      localStorage.setItem("storyProgressState", serialized);
    }
  ...
  try {
    const initialFlags: FlagData[] = [
      ...
      },
      {
        ...
        category: "quest",
        priority: 5,
        description: "Player has obtained a map",
      },
    ...
    console.log(
      "[StoryProgress] Story progress initialized with Gorstan-specific flags",
    );
  } catch (error) {
...
export function getProgressStatistics(): ProgressStatistics {
  try {
    ...
    const highPriorityFlags = Array.from(flags)
      ...
      .filter(
        (item) => item.meta && item.meta.priority && item.meta.priority > 5,
      )
      .sort((a, b) => {
        return (b.meta?.priority || 0) - (a.meta?.priority || 0);
      })
...
export function clearAllProgress(confirm: boolean = false): boolean {
  try {
    if (!confirm) {
      console.warn(
        "[StoryProgress] clearAllProgress requires explicit confirmation",
      );
      return false;
    }
...
function validateCategory(category: string): string {
  ...
  return validCategories.includes(category as FlagCategory)
    ? category
    : "general";
}
