...
const sceneCache = new Map<
  string,
  { scene: Scene; timestamp: number; accessCount: number }
>();
const sceneStats = {
  cacheHits: 0,
  cacheMisses: 0,
...
export class SceneFlowAnalyzer {
    ...
    for (const [sceneId, scene] of this.sceneRegistry) {
      ...
      if (scene.choices) {
        const blockedChoices = scene.choices.filter(
          (choice) =>
            choice.requirements &&
            this.hasComplexRequirements(choice.requirements),
        );
        ...
        if (blockedChoices.length > 0) {
          bottlenecks.push({
            ...
            severity:
              blockedChoices.length === scene.choices.length
                ? "critical"
                : "medium",
  ...
  private calculateAveragePathLength(): number {
    ...
    return totalExecutions > 0
      ? (this.executionHistory.size / totalExecutions) * 3
      : 0;
  }
  ...
  private calculateOptimalOrder(currentOrder: string[]): string[] {
    ...
    return [...currentOrder].sort((a, b) => {
      ...
      const priorityA = sceneA
        ? categoryPriority[sceneA.category || "exploration"] || 2
        : 10;
      const priorityB = sceneB
        ? categoryPriority[sceneB.category || "exploration"] || 2
        : 10;
...
export class SceneRecommendationEngine {
  ...
  calculateSceneRelevance(sceneId: string, context: SceneContext): number {
    ...
    if (scene.category === "tutorial" && !context.player?.experience) {
      ...
      context.player?.traits?.includes("social")
    ) {
      relevance += 10;
    }
        ...
        choices: [
          {
            ...
            action: (ctx) => {
              ctx.appendMessage(
                "🌿 You carefully gather the herbs and feel refreshed.",
              );
        ...
        choices: [
          {
            ...
            action: (ctx) => {
              ctx.appendMessage(
                "🧩 Your experience pays off! You solve the complex puzzle.",
              );
              ctx.setGameState((prev) => ({
                ...prev,
  ...
  private checkConditions(
    ...
    return conditions.every((condition) => {
      switch (condition.type) {
        ...
        case "hasItem":
          return (
            context.player?.inventory?.includes(condition.value as string) ||
            false
          );
        case "hasTrait":
          return (
            context.player?.traits?.includes(condition.value as string) || false
          );
        case "healthAbove":
...
export class ScenePerformanceMonitor {
  ...
  identifySlowScenes(): PerformanceIssue[] {
    ...
    for (const [sceneId, data] of this.performanceData.entries()) {
      ...
      const averageTime =
        data.reduce((sum, entry) => sum + entry.duration, 0) / data.length;
  ...
  optimizeSceneExecution(): OptimizationReport {
    ...
    return {
      ...
      optimizedScenes: slowScenes.map(
        (issue: PerformanceIssue) => issue.sceneId,
      ),
      recommendations: [
  ...
  registerScene({
    ...
    choices: [
      ...
      },
      {
        ...
        action: (context: SceneContext) => {
          context.appendMessage(
            "🐟 You decide to leave Dominic in peace. He seems grateful.",
          );
          context.setGameState((prev) => ({
            ...prev,
            flags: {
  ...
  registerScene({
    ...
    choices: [
      ...
      },
      {
        ...
        action: (context: SceneContext) => {
          context.appendMessage(
            "🔍 You find a mysterious scroll hidden behind the books.",
          );
          context.setGameState((prev) => ({
            ...prev,
...
export function runScene(
  ...
  try {
    ...
    if (scene.conditions && !checkSceneConditions(scene.conditions, context)) {
      ...
      context.appendMessage(
        "❌ You cannot access this scene right now.",
        "warning",
      );
      return result;
    }
    ...
    if (!canExecuteScene(sceneId, scene)) {
      ...
      context.appendMessage(
        "⏰ You must wait before accessing this scene again.",
        "warning",
      );
      return result;
    }
    ...
    if (
      scene.dependencies &&
      !checkSceneDependencies(scene.dependencies, context)
    ) {
    ...
    if (scene.actions && scene.actions.length > 0) {
      ...
      if (actionsResult.errors.length > 0) {
        result = {
          ...result,
          warnings: [...(result.warnings || []), ...actionsResult.errors],
        };
      }
    }
    ...
    context.appendMessage(
      "💥 An error occurred while running the scene.",
      "error",
    );
    return result;
  }
  ...
  try {
    return conditions.every((condition) => {
      ...
      switch (condition.type) {
        ...
        case "hasTrait":
          const hasTrait =
            context.player?.traits?.includes(condition.value as string) ||
            false;
          result = evaluateCondition(
            hasTrait,
            true,
        ...
        case "visitCount":
          ...
          result = evaluateCondition(
            visitCount,
            1,
            condition.operator || "greaterEqual",
          );
          break;
        ...
        default:
          console.warn(
            `[SceneEngine] Unknown condition type: ${condition.type}`,
          );
          result = true;
      }
...
function executeSceneActions(
  ...
  const result: {
    ...
    errors: string[];
    stateChanges: Record<string, unknown>;
  } = { executed: 0, errors: [], stateChanges: {} };
        ...
        switch (action.type) {
          ...
          case "addTrait":
            context.setGameState((prev) => ({
              ...prev,
              traits: [
                ...new Set([...(prev.traits || []), action.value as string]),
              ],
            }));
          ...
          case "updateHealth":
            context.setGameState((prev) => {
              const newHealth = Math.max(
                0,
                (prev.health || 100) + (action.value as number),
              );
          ...
          default:
            result.errors.push(
              `Unknown action type: ${action.type} at index ${index}`,
            );
        }
      } catch (error) {
        result.errors.push(
...
function displaySceneChoices(
  ...
  try {
    ...
    const availableChoices = choices.filter((choice) => {
      ...
      if (
        choice.requirements &&
        !checkChoiceRequirements(choice.requirements, context)
      )
        {return false;}
...
function checkChoiceRequirements(
  ...
  try {
    ...
    if (
      requirements.flags &&
      !requirements.flags.every(
        (flag) => context.flags?.[flag] || context.player?.flags?.[flag],
      )
    )
      {return false;}
    ...
    if (
      requirements.traits &&
      !requirements.traits.every((trait) =>
    ...
    if (
      requirements.health &&
      (context.player?.health || 0) < requirements.health
    ) {
      return false;
    }
    ...
    if (
      requirements.score &&
      (context.player?.score || 0) < requirements.score
    ) {
      return false;
    }
    ...
    if (
      requirements.level &&
      (context.player?.level || 0) < requirements.level
    ) {
      return false;
    }
...
function updateSceneExecutionTracking(
  ...
  try {
    ...
    sceneStats.mostExecutedScenes[sceneId] =
      (sceneStats.mostExecutedScenes[sceneId] || 0) + 1;
  ...
  try {
    ...
    if (choice.condition && !choice.condition(context)) {
      ...
      return {
        ...result,
        messagesAdded: 1,
        warnings: ["Choice condition not met"],
      };
    }
    ...
    if (
      ...
      const missing = getChoiceRequirementsMissing(
        choice.requirements,
        context,
      );
      context.appendMessage(
    ...
    if (choice.nextScene) {
      ...
      result = {
        ...
        actionsExecuted:
          result.actionsExecuted + nextSceneResult.actionsExecuted,
      };
    ...
    if (filter) {
      sceneIds = sceneIds.filter((id) => {
        ...
        if (filter.tags && filter.tags.length > 0) {
          if (
            !scene.tags ||
            !filter.tags.some((tag) => scene.tags!.includes(tag))
          ) {
            return false;
          }
        }
...
export function getSceneExecutionHistory(): Array<{
  ...
  try {
    return Array.from(sceneExecutionHistory.entries()).map(
      ([sceneId, data]) => ({
        sceneId,
        count: data.count,
