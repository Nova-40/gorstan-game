...
export function handlePlayerDeath(
  ...
  try {
    ...
    const messages = generateDeathMessages(
      cause,
      deathStats,
      playerState,
      context,
    ...
    const specialEffects = calculateDeathEffects(
      cause,
      deathStats,
      playerState,
    );
    ...
    const updatedFlags = {
      ...
      [`death_${cause}_count`]:
        ((playerState.flags?.[`death_${cause}_count`] as number) || 0) + 1,
    };

    // Calculate score adjustments
    const scoreAdjustment = calculateScoreAdjustment(
      cause,
      deathStats,
      specialEffects,
    );
...
function generateDeathMessages(
  ...
  messages.push(
    `This is your ${getOrdinalNumber(deathStats.totalDeaths)} death.`,
  );
  ...
  switch (cause) {
    ...
    case "glitch":
      effects.push({
        type: "special_unlock",
        description:
          "Your brush with digital death grants insight into the simulation.",
...
function getFailsafeDeathResult(
  playerState: PlayerState,
  cause: DeathCause,
): DeathResult {
  return {
 ...
 */
export function hasPlayerDiedFrom(
  cause: DeathCause,
  playerState: PlayerState,
): boolean {
  const deathsByCause =
