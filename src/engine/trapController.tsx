...
const currentActiveTrap: TrapData | null = null;
...
const trapCache: Map<string, { exists: boolean; timestamp: number }> =
  new Map();
...
export function seedTraps(
  ...
  try {
    ...
    roomKeys.forEach((roomId) => {
      try {
        if (
          seedConfig.roomExclusions.some((exclusion) =>
            roomId.includes(exclusion),
          )
        ) {
          return;
        }
        ...
        console.error(
          `[TrapController] Error seeding trap in room ${roomId}:`,
          error,
        );
      }
    });
    ...
    console.log(
      `[TrapController] Traps seeded: ${trapsSeeded} out of ${roomKeys.length} rooms`,
    );
...
export function triggerTrap(
  ...
  try {
    ...
    if (
      trap.requirements &&
      !checkTrapRequirements(trap.requirements, context.playerState)
    ) {
      return null;
    }
...
export function disarmTrap(
  ...
  try {
    ...
    console.log(
      `[TrapController] Trap disarmed in ${roomName} using ${method}`,
    );
    return true;
  } catch (error) {
...
export function isRoomTrapped(roomName: string): boolean {
  try {
    ...
    return (
      trap !== null &&
      (!(trap.triggered ?? false) || (trap.repeatTrigger ?? false))
    );
  } catch (error) {
...
export function clearAllTraps(confirm: boolean = false): boolean {
  try {
    if (!confirm) {
      console.warn(
        "[TrapController] clearAllTraps requires explicit confirmation",
      );
      return false;
    }
...
function checkTrapRequirements(
  ...
  try {
    if (
      ...
      (playerState.level || 1) < requirements.minLevel
    ) {
      return false;
    }
  ...
  try {
    ...
    if (playerTraits.includes("trap_expert")) {
      return {
        ...
        method: "expertise",
        message:
          "🎯 Your expertise allows you to expertly disarm the trap before it can harm you.",
      };
    }
    ...
    if (
      ...
      return {
        ...
        method: "resistance",
        message:
          "🛡️ Your natural resistance protects you from the trap's effects.",
      };
    }
    ...
    if (playerItems.includes("trapkit") || playerItems.includes("trap kit")) {
      return {
        ...
        method: "trapkit",
        message:
          "🔧 You quickly deploy your trap kit to neutralize the danger.",
      };
    }
    ...
    if (
      ...
      return {
        ...
        method: "tools",
        message:
          "🔓 Your thieves' tools allow you to carefully disarm the mechanism.",
      };
    }
    ...
    if (trap.type === "magical" && playerTraits.includes("mage")) {
      return {
        ...
        method: "magic",
        message:
          "✨ Your magical knowledge allows you to dispel the trap's enchantment.",
      };
    }
