...
function getNPCResponse(npcId: string, context: any): string {
  // Placeholder implementation
  return `${npcId} nods at you.`;
}
...
function handleSpeaking(target: string, context: any): CommandResult {
  ...
  if (!target) {
    ...
    if (npcs.length > 0) {
      responses.push(`You can speak to: ${npcs.join(', ')}`);
    } else {
  ...
  if (!currentRoom.npcs?.includes(target)) {
  ...
  specialActions.push({
    ...
    data: { npc: target, room: currentRoom.id }
  });
