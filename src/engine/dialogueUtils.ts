...
export function checkProbability(threshold: number): boolean {
  if (threshold < 0 || threshold > 1) {
    console.warn(
      "[DialogueUtils] Probability threshold must be between 0 and 1",
    );
    return false;
  }
...
export function selectWeightedDialogue(
  ...
  const validVariations = variations.filter(
    (variation) =>
      ...
      checkDialogueConditions(variation.conditions, playerState || {}),
  );
  ...
  const totalWeight = validVariations.reduce(
  ...
  const moodVariations: Record<string, DialogueVariation[]> = {
    ...
    hostile: [
      { text: "What do you want?", weight: 2 },
      {
        ...
        weight: 2,
        conditions: [
          { type: "trust", key: "trust", value: 0, operator: "less" },
        ],
      },
    ...
    caring: [
      {
        ...
        weight: 3,
        conditions: [
          { type: "stat", key: "deathCount", value: 2, operator: "greater" },
        ],
      },
      {
        ...
        weight: 2,
        conditions: [
          { type: "trust", key: "trust", value: 4, operator: "greater" },
        ],
      },
