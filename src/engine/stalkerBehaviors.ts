    ...
    setTimeout(() => {
      if (wendell.currentRoom !== stalkerState.lastSeenRoom) {
        ...
        if (
          ...
          const stalkerMessage = getWendellStalkerMessage(
            stalkerState.intensity,
          );
...
export function handlePollyStalking(
  ...
  if (state.currentRoomId !== stalkerState.lastSeenRoom) {
    ...
    setTimeout(() => {
      ...
      const targetRoom = shouldAppearInSameRoom
        ? state.currentRoomId
        : stalkerState.lastSeenRoom;
...
function handlePollyEscalation(
  ...
  dispatch({
    type: "SET_FLAG",
    payload: { flag: "pollyVengeanceEscalating", value: true },
  });
  dispatch({
    type: "SET_FLAG",
