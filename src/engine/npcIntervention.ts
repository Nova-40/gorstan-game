...
export const ConditionHelpers = {
  hasTrait: (
    ...
    try {
      ...
      console.error(
        `[NPCIntervention] Error checking trait ${traitName}:`,
        error,
      );
      return false;
    }
  ...
  hasItem: (
    ...
    try {
      ...
      console.error(
        `[NPCIntervention] Error checking item ${itemName}:`,
        error,
      );
      return false;
    }
  ...
  inRoom: (currentRoom: string | undefined, roomPattern: string): boolean => {
    try {
      ...
      console.error(
        `[NPCIntervention] Error checking room ${roomPattern}:`,
        error,
      );
      return false;
    }
  ...
  hasFlag: (flags: Record<string, unknown>, flagName: string): boolean => {
    try {
      ...
      console.error(
        `[NPCIntervention] Error checking flag ${flagName}:`,
        error,
      );
      return false;
    }
...
const interventionRules: InterventionRule[] = [
  {
    ...
    conditions: (context: InterventionContext) => {
      try {
        ...
        console.error(
          "[NPCIntervention] Error in albie_peacekeeper condition:",
          error,
        );
        return false;
      }
  ...
  },
  {
    ...
    conditions: (context: InterventionContext) => {
      try {
        ...
        console.error(
          "[NPCIntervention] Error in ayla_scholar_mediation condition:",
          error,
        );
        return false;
      }
  ...
  },
  {
    ...
    conditions: (context: InterventionContext) => {
      try {
        ...
        console.error(
          "[NPCIntervention] Error in al_earth_calming condition:",
          error,
        );
        return false;
      }
  ...
  },
  {
    ...
    messages: {
      intervention:
        "Your reputation precedes you, and the NPCs show you respect.",
      suppressed: [
  ...
  },
  {
    ...
    conditions: (context: InterventionContext) => {
      try {
        ...
        console.error(
          "[NPCIntervention] Error in polly_dominic_protection condition:",
          error,
        );
        return false;
      }
    ...
    messages: {
      intervention:
        "Polly steps protectively in front of Dominic: 'Don't you dare!'",
      suppressed: [
    ...
    conditions: (context: InterventionContext) => {
      try {
        ...
        return (
          ...
          ConditionHelpers.hasFlag(
            context.flags,
            "academic_protocol_violated",
          ) ||
        ...
        console.error(
          "[NPCIntervention] Error in wendell_authority_assertion condition:",
          error,
        );
        return false;
      }
  ...
  },
  {
    ...
    conditions: (context: InterventionContext) => {
      try {
        ...
        console.error(
          "[NPCIntervention] Error in morthos_cynical_observation condition:",
          error,
        );
        return false;
      }
  ...
  },
  {
    ...
    conditions: (context: InterventionContext) => {
      try {
        ...
        console.error(
          "[NPCIntervention] Error in dominic_innocent_defusion condition:",
          error,
        );
        return false;
      }
    ...
    messages: {
      intervention:
        "Dominic asks innocently: 'Are we playing a game? Can I play too?'",
      suppressed: [
...
export function checkNPCInterventions(
  ...
  try {
    ...
    for (const rule of availableRules) {
      try {
        ...
        const targetNPCsLower = rule.targetNPCs.map((n) =>
          n.toLowerCase().trim(),
        );
        ...
        console.error(
          `[NPCIntervention] Error processing rule ${rule.id}:`,
          ruleError,
        );
        continue;
      }
...
function canTriggerIntervention(
  ...
  try {
    ...
    console.error(
      "[NPCIntervention] Error checking intervention trigger:",
      error,
    );
    return false;
  }
...
function updateInterventionHistory(rule: InterventionRule, now: number): void {
  try {
    ...
    console.error(
      "[NPCIntervention] Error updating intervention history:",
      error,
    );
  }
}
...
function trackRecentIntervention(
  ...
  try {
    ...
    if (recentInterventions.length > MAX_RECENT_INTERVENTIONS) {
      recentInterventions.splice(
        0,
        recentInterventions.length - MAX_RECENT_INTERVENTIONS,
      );
    }
  } catch (error) {
    console.error(
      "[NPCIntervention] Error tracking recent intervention:",
      error,
    );
  }
}
...
export function removeInterventionRule(ruleId: string): boolean {
  try {
    ...
    if (index >= 0) {
      ...
      const filteredRecent = recentInterventions.filter(
        (r) => r.ruleId !== ruleId,
      );
      recentInterventions.splice(
        0,
...
export function getInterventionHistory(): Record<string, InterventionHistory> {
  try {
    ...
    console.error(
      "[NPCIntervention] Error getting intervention history:",
      error,
    );
    return {};
  }
...
export function resetInterventionHistory(): void {
  try {
    ...
    console.error(
      "[NPCIntervention] Error resetting intervention history:",
      error,
    );
  }
}
...
export function canNPCIntervene(
  ...
  try {
    if (
      ...
      !validateInterventionContext(context)
    ) {
      return false;
    }
    ...
    const applicableRules = interventionRules.filter(
      (rule) =>
        rule.interventionNPC.toLowerCase().trim() ===
    ...
    console.error(
      "[NPCIntervention] Error checking if NPC can intervene:",
      error,
    );
    return false;
  }
...
export function getPotentialInterventions(context: InterventionContext): Array<{
  ...
  try {
    ...
    return interventionRules
      .map((rule) => {
        const lowerNpcs = context.npcsInRoom.map((npc) =>
          npc.toLowerCase().trim(),
        );
        const isNPCPresent = lowerNpcs.includes(
          rule.interventionNPC.toLowerCase().trim(),
        );
        const suppressedNPCs = findSuppressedNPCs(rule, lowerNpcs);
    ...
    console.error(
      "[NPCIntervention] Error getting potential interventions:",
      error,
    );
    return [];
  }
  ...
  try {
    ...
    if (recentInterventions.length > 1) {
      for (let i = 1; i < recentInterventions.length; i++) {
        intervals.push(
          recentInterventions[i].timestamp -
            recentInterventions[i - 1].timestamp,
        );
      }
