...
import {
  MiniquestData,
  MiniquestProgress,
} from "../components/MiniquestInterface";

import { MiniquestEngine } from "../engine/miniquestInitializer";

import {
  aiMiniquestService,
  AIMiniquestRecommendation,
} from "../services/aiMiniquestService";
...
class MiniquestController {
  ...
  public async openMiniquestInterface(
    ...
    miniquests.forEach((quest) => {
      ...
      // Variable declaration
      const isAvailable =
        availableQuests.find((q) => q.id === quest.id) !== undefined;
      // Variable declaration
  ...
  public async attemptQuest(
    ...
    try {
      ...
      if (this.aiEnabled) {
        try {
          const quest = this.getAllRoomQuests(roomId).find(
            (q) => q.id === questId,
          );
          if (quest) {
            const adaptation = await aiMiniquestService.getAdaptedDifficulty(
              quest,
              gameState,
            );
      ...
      if (result.success && this.dispatch) {
        ...
        if (result.scoreAwarded) {
          this.dispatch({
            type: "UPDATE_SCORE",
            payload: {
              score: (gameState.player.score || 0) + result.scoreAwarded,
            },
          } as any);
        }
  ...
  public getGlobalStats(gameState: LocalGameState): {
    ...
    Object.entries(miniquestState).forEach(
      ([roomId, roomState]: [string, any]) => {
    ...
    allRoomIds.forEach((roomId) => {
      // Variable declaration
      const availableQuests = engine.getAvailableQuests(
        roomId,
        gameState as any,
      );
