...
function handleGreasyNapkin(
  ...
  dispatch({
    type: "SET_FLAG",
    payload: { key: "hasUnlockedScrolls", value: true },
  });
...
export function handleGuardQuestion(
  ...
  const doors = ["red", "blue", "green"];
  // Variable declaration
  const wrongDoors = doors.filter(
    (door) => door !== librarianState.correctDoor,
  );
  // Variable declaration
  const guardResponse =
...
export function handleDoorChoice(
  ...
  if (isCorrect) {
    ...
    dispatch({
      type: "SET_FLAG",
      payload: { key: "hasSolvedLibraryPuzzle", value: true },
    });
