...
export const mrWendellDialogue: DialogueTree = {
  ...
  pity: [
    {
      text: "Manners matter. So few show them. I'll let you pass... this once.",
      chance: 0.1,
    },
    {
...
export function getWanderingNPCResponse(
  ...
  if (
    npcId === "albie" &&
    playerState.name?.toLowerCase() === "geoff" &&
    topic === "greeting"
  ) {
...
export async function getEnhancedWanderingNPCResponse(
  ...
  if (playerInput) {
    try {
      ...
      console.warn(
        "Enhanced wandering NPC response failed, using fallback:",
        error,
      );
      // Fall back to standard response
      const standardResponse = getWanderingNPCResponse(
        npcId,
        topic,
        playerState,
        npcState,
...
export function getWanderingNPCIdleLine(
  npcId: string,
  playerState: PlayerState,
): string | null {
...
export function getNPCInteractionLine(
  npc1: string,
  npc2: string,
): string | null {
  const interactions: Record<string, Record<string, string[]>> = {
    ayla: {
      ...
      morthos: [
        "*Ayla's light pushes back the shadows* Not today, old friend.",
      ],
    },
    albie: {
      morthos: [
        "*Albie steps forward professionally* I'm going to need you to move along, sir.",
      ],
      polly: [
        "*Albie clears his throat* Ma'am, I think it's time you found somewhere else to be.",
      ],
