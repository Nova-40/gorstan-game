...
export interface TrapDefinition {
  ...
  autoDisarm?: boolean;
  cooldown?: number;
  traps?: TrapDefinition[]; // Added traps property
}
...
const trapCache: Map<string, { result: boolean; timestamp: number }> =
  new Map();
...
export function seedTraps(
  ...
  try {
    if (!Array.isArray(roomIds) || roomIds.length === 0) {
      console.warn(
        "[TrapEngine] Warning: No valid room IDs provided for seeding traps.",
      );
      return;
    }
    ...
    const validRooms = roomIds.filter(
      (id) =>
        !seedConfig.excludeRooms.includes(id) &&
        typeof id === "string" &&
        id.length > 0,
    );
    ...
    if (validRooms.length === 0) {
      console.warn(
        "[TrapEngine] No valid rooms available for trap seeding after exclusions.",
      );
      return;
    }

    // Separate preferred and other rooms
    const preferredRooms = validRooms.filter((id) =>
      seedConfig.preferredRooms.includes(id),
    );
    const otherRooms = validRooms.filter(
      (id) => !seedConfig.preferredRooms.includes(id),
    );
    ...
    const preferredTraps = Math.min(
      Math.floor(maxTraps * 0.4),
      shuffledPreferred.length,
    );
    const remainingTraps = Math.min(
      maxTraps - preferredTraps,
    ...
    if (debugMode) {
      console.info(
        `🧨 Traps seeded: ${trapsSeeded} in rooms:`,
        Object.keys(seededTraps),
      );
    }
  } catch (error) {
...
export function handleRoomTrap(
  ...
  try {
    ...
    const disarmResult = checkDisarmAbility(
      trap,
      playerState,
      playerItems,
      playerTraits,
    ...
    if (
      playerTraits.includes("resistant") ||
      playerTraits.includes("trap_resistant")
    ) {
...
function checkDisarmAbility(
  ...
  try {
    ...
    if (
      playerTraits.includes("trap_expert") ||
      playerTraits.includes("master_thief")
    ) {
      return {
        canDisarm: true,
    ...
    if (
      playerItems.includes("thieves_tools") ||
      playerItems.includes("lockpicks")
    ) {
      return {
        canDisarm: true,
...
export function handleTrapEscape(
  roomId: string,
  playerState?: PlayerState,
): boolean {
  try {
...
export function maybeTriggerInquisitionTrap(
  ...
  try {
    ...
    if (Math.random() < 0.05) {
      ...
      if (
        commandText.includes("expect") ||
        commandText.includes("inquisition")
      ) {
        appendMessage(
          `😲 They're baffled by your cleverness and award you a certificate.`,
        );
        ...
        appendMessage(
          `You sit in the comfy chair. Gain +1 health, lose -1 dignity.`,
        );
...
export function maybeTriggerBugblatterTrap(
  ...
  try {
    ...
    if (Math.random() < 0.0142) {
      appendMessage(
        `🌌 You feel a strange presence. Something *very stupid* is watching you.`,
      );
      appendMessage(
        `💥 A voice booms: "Beware the Ravenous Bugblatter Beast of Traal!"`,
      );
      ...
      const hasTowel = playerInventory.some(
        (item) =>
          item &&
          typeof item === "string" &&
          item.toLowerCase().includes("towel"),
      );
      ...
      if (hasTowel) {
        ...
        appendMessage(
          `🧠 As Douglas Adams rightly pointed out, towels are invaluable for travel.`,
        );
        ...
        appendMessage(
          `😱 The beast slobbers all over your narrative. You lose 2 health.`,
        );
        if (typeof playerState.health === "number") {
...
export function getTrapCount(filter?: {
  ...
  try {
    ...
    return Object.values(seededTraps).filter((trap) => {
      if (
        filter.triggered !== undefined &&
        trap.triggered !== filter.triggered
      ) {
        return false;
      }
...
export function getTrapStatistics(): TrapEngineStats & {
  ...
  try {
    const activeTraps = Object.values(seededTraps).filter(
      (trap) => !trap.triggered,
    );
