...
import MiniquestController, {
  MiniquestControllerResult,
} from "../engine/miniquestController";
  ...
  switch (verb) {
    ...
    case "miniquests": {
      if (!noun || noun === "list") {
        ...
        return {
          messages: [
            ...
            createGameMessage(
              `Found ${interfaceData.miniquests.length} quests in this area`,
            ),
            createGameMessage(
      ...
      if (noun.startsWith("attempt ")) {
        ...
        return await handleMiniquestAttempt(
          questId,
          currentRoomId,
          gameState,
          miniquestController,
      ...
      if (noun === "stats" || noun === "statistics") {
        ...
        return {
          messages: [
            ...
            createGameMessage(
              `Available Quests: ${globalStats.totalAvailable}`,
            ),
          ],
        };
      }
      ...
      if (noun === "progress") {
        // Variable declaration
        const roomProgress = miniquestController.getRoomProgress(
          currentRoomId,
          gameState,
        );
        return {
          messages: [
            createGameMessage(
              `🎯 **Miniquest Progress for ${currentRoomId}:**`,
            ),
            createGameMessage(
              `Completed: ${roomProgress.completed}/${roomProgress.total}`,
            ),
      ...
      return {
        messages: [
          ...
          createGameMessage(
            "  miniquests attempt [name] - Try a specific quest",
          ),
    ...
    case "quests": {
      ...
      return {
        messages: [
          ...
          createGameMessage(
            "Enhanced miniquest browser with filtering and details",
          ),
        ],
        miniquestModal: interfaceData,
      };
      ...
      if (roomProgress.total > 0) {
        return {
          messages: [
            ...
            createGameMessage(
              '• Use "miniquests" to open the enhanced quest browser',
            ),
            createGameMessage(
              "• Check quest requirements - you might need specific items",
            ),
            createGameMessage(
              "• Look for environmental cues in room descriptions",
            ),
            createGameMessage(
              "• Some quests become available after story progression",
            ),
            createGameMessage(
              "• Completed quests award score and may unlock new areas",
            ),
            createGameMessage(
              "• Try examining, listening, or interacting with objects",
            ),
          ],
        };
      }
    ...
    case "objectives": {
      ...
      const availableQuests = interfaceData.miniquests.filter(
        (q) =>
          interfaceData.progress[q.id]?.available &&
          !interfaceData.progress[q.id]?.completed,
      );
      ...
      availableQuests.slice(0, 3).forEach((quest) => {
        messages.push(
          createGameMessage(`• ${quest.title} - ${quest.description}`),
        );
...
async function handleMiniquestAttempt(
  ...
  try {
    // Variable declaration
    const result = await miniquestController.attemptQuest(
      questId,
      currentRoomId,
      gameState,
    );
    ...
    if (result.success && result.scoreAwarded) {
      messages.push(
        createGameMessage(
          `🏆 Quest completed! +${result.scoreAwarded} points`,
          "achievement",
        ),
      );
    }
    ...
    return {
      messages: [
        createGameMessage(
          `Miniquest "${questId}" not found in this area.`,
          "error",
        ),
        createGameMessage(
          'Use "miniquests" to see available quests.',
          "system",
        ),
      ],
    };
...
export function getMiniquestContextualInfo(
  roomId: string,
  gameState: LocalGameState,
): string[] {
  // Variable declaration
  ...
  if (progress.total > 0) {
    ...
    if (progress.completed > 0) {
      messages.push(
        `✅ ${progress.completed}/${progress.total} miniquests completed in this area`,
      );
    }

    if (progress.available > 0 && progress.completed === 0) {
      messages.push(
        `💡 Look around carefully - this area holds secrets waiting to be discovered`,
      );
    }
  }
...
export function getMiniquestHelpMessages(): GameMessage[] {
  return [
    createGameMessage("--- Miniquest Commands ---"),
    createGameMessage(
      "miniquests - Open enhanced quest interface with filtering",
    ),
    ...
    createGameMessage(
      "• Quest types: dynamic, structured, puzzle, social, exploration",
    ),
    ...
    createGameMessage(
      "• Completed quests award score and may unlock new content",
    ),
  ];
}
