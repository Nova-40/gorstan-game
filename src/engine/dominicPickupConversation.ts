...
export function getPickupConversationState(
  playerId: string,
): PickupConversationState {
...
export function handleDominicPickupAttempt(
  ...
  if (convState.attempts >= 4 || convState.playerInsisted) {
    ...
    dispatch({
      type: "SET_FLAG",
      payload: { flag: "dominicTakenAfterWarning", value: true },
    });
    dispatch({
      type: "SET_FLAG",
...
export function getDominicResponse(
  context: "philosophical" | "warning" | "wise" | "sad",
): string {
