...
function checkTrapDetection(
  ...
  if (
    ...
    playerTraits.includes("perceptive") ||
    playerTraits.includes("alert")
  ) {
    detectionChance += 0.6;
  ...
  if (
    playerItems.includes("trap_detector") ||
    playerItems.includes("scanner")
  ) {
    detectionChance += 0.7;
...
export function searchForTraps(
  ...
  return {
    detected: false,
    warning:
      "🔍 **Searching carefully...** You find no traps in this area. It appears safe.",
  };
}
