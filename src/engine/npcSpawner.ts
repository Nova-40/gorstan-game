...
export const wanderingNPCs: WanderingNPC[] = [
  {
    ...
    exitLine:
      "Mr. Wendell fades back into the darkness with an almost imperceptible nod.",
    personality: {
  ...
  },
  {
    ...
    name: "The Librarian",
    entryLine:
      "A scholarly figure emerges from the shadows between towering bookshelves.",
    exitLine:
  ...
  },
  {
    ...
    name: "Ayla",
    entryLine:
      "A warm presence fills the room as Ayla materializes, her form shimmering slightly.",
    exitLine:
      "Ayla fades away with a gentle smile, leaving only a sense of understanding.",
    personality: {
...

  {
    ...
    name: "Al",
    entryLine:
      "A bureaucratic figure in a slightly rumpled suit appears, adjusting his glasses.",
    exitLine:
      "Al checks his watch and fades away with bureaucratic efficiency.",
    personality: {
...

  {
    ...
    exitLine:
      "The fishbowl fades away, but Dominic's knowing look lingers in your memory.",
    personality: {
...

  {
    ...
    exitLine:
      "Polly backs away slowly, never breaking eye contact until she fades from view.",
    personality: {
...

  {
    ...
    exitLine:
      "Albie tips his cap and walks away with the measured pace of security personnel.",
    personality: {
...
function canNPCSpawnInRoom(
  ...
  if (
    npc.zonePreferences.length > 0 &&
    !npc.zonePreferences.includes(roomZone)
  ) {
    return false;
  }
...
function evaluateSpawnCondition(
  ...
  switch (condition.type) {
    ...
    case "zone":
      ...
      return (
        zone === condition.value || zone.includes(condition.value as string)
      );
...
export function shouldSpawnNPC(npc: WanderingNPC, state: PlayerState): boolean {
  if (
    (npc as any).questOnly &&
    state.flags &&
    !(state.flags as any)[(npc as any).requiredFlag]
  )
    {return false;}
  return true;
}
