...
import PuzzleController, {
  PuzzleControllerResult,
} from "../engine/puzzleController";
...
export async function processPuzzleCommand(
  ...
  switch (verb) {
    ...
    case "puzzles": {
      if (!noun || noun === "list") {
        // Variable declaration
        const puzzleMessages = puzzleController.listRoomPuzzles(
          currentRoomId,
          gameState,
        );
        return {
      ...
      if (noun.startsWith("solve ")) {
        ...
        return await handlePuzzleSolve(
          puzzleId,
          currentRoomId,
          gameState,
          puzzleController,
      ...
      if (noun.startsWith("progress")) {
        // Variable declaration
        const progress = puzzleController.getRoomPuzzleProgress(
          currentRoomId,
          gameState,
        );
        return {
    ...
    case "hint": {
      // Variable declaration
      const puzzles = puzzleController.listRoomPuzzles(
        currentRoomId,
        gameState,
      );
      ...
      if (puzzles.length > 1) {
        return {
          messages: [
            ...
            createGameMessage(
              '• Use "puzzle list" to see all available puzzles in this area',
            ),
            createGameMessage(
              "• Check puzzle requirements - you might need specific items or traits",
            ),
            createGameMessage(
              "• Some puzzles have limited attempts, so think carefully!",
            ),
            createGameMessage(
              "• The puzzle interface will provide hints once you start solving",
            ),
            createGameMessage(
...
async function handlePuzzleSolve(
  ...
  // Variable declaration
  const canStart = puzzleController.canStartPuzzle(
    puzzleId,
    currentRoomId,
    gameState,
  );
  ...
  try {
    // Variable declaration
    const puzzleSession = await puzzleController.startPuzzle(
      puzzleId,
      currentRoomId,
      gameState,
    );
    ...
    if (!puzzleSession.showPuzzleModal || !puzzleSession.puzzle) {
      return {
        messages: [
          createGameMessage(
            `Puzzle "${puzzleId}" not found in this area.`,
            "error",
          ),
    ...
    return {
      messages: [
        ...
        createGameMessage(
          `Difficulty: ${puzzleSession.puzzle.difficulty.toUpperCase()}`,
        ),
    ...
    return {
      messages: [
        createGameMessage(
          "An error occurred while starting the puzzle.",
          "error",
        ),
      ],
    };
...
export function getPuzzleContextualInfo(
  roomId: string,
  gameState: LocalGameState,
): string[] {
  // Variable declaration
  ...
  if (progress.total > 0) {
    ...
    if (progress.solved > 0) {
      messages.push(
        `✅ ${progress.solved}/${progress.total} puzzles completed in this area`,
      );
    }
  }
...
export function getPuzzleHelpMessages(): GameMessage[] {
  return [
    ...
    createGameMessage(
      "• Different puzzle types: logic, pattern, navigation, sequence",
    ),
    ...
    createGameMessage(
      "• Use hints wisely - they can guide you to the solution",
    ),
    createGameMessage(
      "• Completing puzzles rewards score, items, and achievements",
    ),
  ];
}
