...
export function generateTrapReport(roomMap: Record<string, TrapDefinition>): {
  totalRooms: number;
  ...
  for (const [roomId, room] of Object.entries(roomMap)) {
    if (room.traps && Array.isArray(room.traps) && room.traps.length > 0) {
      ...
      const hasLethalTrap = room.traps.some(
        (trap: TrapDefinition) => trap.severity === "lethal",
      );
      ...
      room.traps.forEach((trap: TrapDefinition) => {
        ...
        if (trap.severity)
          {trapsBySeverity[trap.severity] =
            (trapsBySeverity[trap.severity] || 0) + 1;}
      });
    }
  }
  ...
  const averageTrapsPerRoom =
    roomsWithTraps > 0 ? totalTraps / roomsWithTraps : 0;
...
export function validateTrap(trap: TrapDefinition): TrapValidationResult {
  ...
  if (!trap.type) {
    ...
    !["damage", "teleport", "item_loss", "flag_set", "custom"].includes(
      trap.type,
    )
  ) {
  ...
  if (!trap.severity) {
    ...
    !["light", "moderate", "severe", "lethal"].includes(trap.severity)
  ) {
...
export function validateTrapSystem(
  roomMap: Record<string, any>,
): TrapSystemValidationResult {
  ...
  for (const [roomId, room] of Object.entries(roomMap)) {
    if (room.traps && Array.isArray(room.traps)) {
      ...
      const trapValidations = room.traps.map((trap: TrapDefinition) =>
        validateTrap(trap),
      );
        ...
        if (validation.issues.length > 0) {
          validation.issues.forEach((issue: string) => {
            result.warnings.push(
              `Room ${roomId}, trap ${validation.trapId}: ${issue}`,
            );
          });
        }
      });
      ...
      if (duplicates.length > 0) {
        result.errors.push(
          `Room ${roomId} has duplicate trap IDs: ${duplicates.join(", ")}`,
        );
        result.isOperational = false;
      }
    }
  ...
  if (result.roomsWithTraps.length === 0) {
    result.warnings.push(
      "No rooms with traps found - trap system may be underutilized",
    );
  }

  if (result.roomsWithTraps.length > 0) {
    // Variable declaration
    const totalTraps = result.roomsWithTraps.reduce(
      (sum, room) => sum + room.trapCount,
      0,
    );
