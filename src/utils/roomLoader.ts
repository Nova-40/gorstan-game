...
function initializeRooms(): void {
  ...
  for (const [, room] of Object.entries(roomRegistry)) {
    if (!room || typeof room.id !== "string") {
      console.warn(
        "[roomLoader] Skipping invalid room entry in registry:",
        room,
      );
      continue;
    }
    ...
    if (roomMap.has(roomId)) {
      console.warn(
        `[roomLoader] Duplicate room ID detected: ${roomId}. Overwriting.`,
      );
    }
  ...
  if (roomMap.size === 0) {
    console.error(
      "[roomLoader] No rooms were loaded! This is a critical error.",
    );
  }
}
...
export function getAllRoomsAsObject(): Record<string, Room> {
  console.log(
    "[roomLoader] getAllRoomsAsObject called, roomMap size:",
    roomMap.size,
  );

  if (roomMap.size === 0) {
    console.error(
      "[roomLoader] roomMap is empty! Attempting to reinitialize...",
    );
    initializeRooms();
  }
  ...
  for (const [id, room] of roomMap.entries()) {
    obj[id] = {
      ...
      npcs: Array.isArray(room.npcs)
        ? room.npcs.map((npc) =>
            typeof npc === "string"
              ? ({ id: npc } as import("../types/Room").RoomNPC)
              : npc,
          )
        : [],
