...
export function validateGlobalItemManagement(
    ...
    if (room.items && Array.isArray(room.items)) {
        ...
        if (!itemData) {
          ...
          if (itemData.spawnRooms && !itemData.spawnRooms.includes(roomId)) {
            ...
            result.warnings.push(
              `Item ${itemId} in room ${roomId} but not in spawnRooms list`,
            );
          }
          ...
          if (itemData.excludeRooms && itemData.excludeRooms.includes(roomId)) {
            ...
            result.errors.push(
              `Item ${itemId} in room ${roomId} but room is excluded`,
            );
            result.isValid = false;
          }
        }
      ...
      itemCounts.forEach((count, itemId) => {
        if (count > 1) {
          ...
          result.warnings.push(
            `Room ${roomId} has duplicate item ${itemId} (${count} times)`,
          );
        }
      });
    }
  ...
  ITEMS.forEach((item) => {
    if (item.spawnRooms && item.excludeRooms) {
      // Variable declaration
      const conflicts = item.spawnRooms.filter((room) =>
        item.excludeRooms!.includes(room),
      );
    ...
    if (item.id === "dominic") {
      if (!item.spawnRooms?.includes("dalesapartment")) {
        ...
        result.warnings.push(
          "Dominic the goldfish should spawn in Dale's apartment",
        );
      }
    }
    ...
    if (item.id === "runbag") {
      if (!item.spawnRooms?.includes("dalesapartment")) {
        result.itemIssues.push({
          ...
          severity: "warning",
          details:
            "Essential storage item should be accessible from Dale's apartment",
        });
  ...
  if (gameState) {
    ...
    inventory.forEach((itemId) => {
      if (inventoryItems.has(itemId)) {
        result.errors.push(
          `Player inventory contains duplicate item: ${itemId}`,
        );
        result.isValid = false;
      } else {
