...
const npcSpecificKnowledge: Record<
  string,
  Record<string, KnowledgeResponse>
> = {
  ayla: {
    "trust advice": {
      responses: [
...
function selectResponse(
  ...
  if (shouldVary && responses.length > 1) {
    ...
    const lastConv = getNPCConversationHistory(state, npcId).entries.slice(
      -1,
    )[0];
    const lastResponse = lastConv?.npcResponse;
    const availableResponses = responses.filter(
      (r) => formatDialogue(r, state) !== lastResponse,
    );
    selectedResponse =
